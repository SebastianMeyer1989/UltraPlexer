		...
		while($line2=<INTWO>){										# parse genome file
			...
			if($line2=~m/^>(.+?) /){									# if header
				if($conca eq ""){									# ...and conca. variable is empty 
					...
					$header2=$1;									### save header-ID
					$counter++;									### save header-number
				}
				else{											# ...and conca. variable is filled
					$sizehash1{$header2}=length($conca);				### save size to header-ID
					$sizehash2{$counter}=length($conca);				### save size to header-number
...
################################################################################################################
		...
		foreach $key(keys(%sizehash2)){									### for all sequences, so for all long-read files (genome and plasmid)
		...
		while($r1line=<INSIX>){								# parse illumina r1 file
			chomp($r1line);
			if($r1line=~m/^@(.+)_(\d+)_\d+_.+_.+_.+/){			### if ref line ($1=ID, $2=read-start)
				if($2>$sizehash1{$1}){						### if read-start is bigger than length of corresponding sequence -> delete read
